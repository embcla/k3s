...
$configureK3SNode=<<-SHELL
  ...
  MYSQL_IP=$(cat /vagrant/mysql_ip)
  IPADDR=$(ip a show enp0s8 | grep "inet " | awk '{print $2}' | cut -d / -f1)
  curl -sfL https://get.k3s.io | INSTALL_K3S_EXEC="agent -v 2 -l /vagrant/${HOSTNAME}.log -t ${NODE_TOKEN} --node-ip=${IPADDR} --flannel-iface=enp0s8 --server https://${MYSQL_IP}:6443" sh -
  sudo sed -i 's/ChallengeResponseAuthentication no/ChallengeResponseAuthentication yes/g' /etc/ssh/sshd_config
  sudo systemctl reload ssh
SHELL
